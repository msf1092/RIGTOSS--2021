function [SBTs, NormNashD] = FncRIGTOSSBqQt(Depth, Q, Bq)
% This function finds the SBT based on the Bq-Qt chart proposed by
% Robertson (1990), using RIGTOSS model!

%% Finding the initial Soil Behavior Type (SBT) of each data based on Robertson (1990):
n = size(Q);
n = n(1,1);
SBT = zeros(n,1);
Q = log10(Q);
% In the following lines, each data point is assigned to a zone of soil
% behavior type (SBT) on Robertson chart:
for i = 1 : n
    e = 1;
    SBT(i,e) = FncSBTpointsLocatorRobertsonBqQt(e, Q(i), Bq(i));
end
...
for i=1:n-1
    D1(i) =(abs((Bq(i))-(Bq(i+1))))^2+(abs((Q(i))-(Q(i+1))))^2;
end
maxD1=max(D1);
for i=1:n
...
for i=1:n-2
    D2(i) =(abs((Bq(i))-(Bq(i+2))))^2+(abs((Q(i))-(Q(i+2))))^2;
end
maxD2=max(D2);
for i=1:n
...
for i=1:n-4
    D3(i) =(abs((Bq(i))-(Bq(i+4))))^2+(abs((Q(i))-(Q(i+4))))^2;
end
maxD3=max(D3);
for i=1:n
...
for i=1:n-10
    D4(i) =(abs((Bq(i))-(Bq(i+10))))^2+(abs((Q(i))-(Q(i+10))))^2;
end
maxD4=max(D4);
for i=1:n
...
X(3,1)=0.3255;
X(4,1)=0.1691;
%to save the Bq and Q values, to be exported into the final file
% Bq1 = Bq;
% Q1 = Q;
%To draw SBTs:
...
for k=1:09 %13 CI levels are considered in "FncAlphaCutDist.m"
    ...
    % Finding Layers and averaging the Bq and Q
    for j=1:2:m-1
        InitInd             = LayerInd(j,1);
        EndInd              = LayerInd(j+1,1);
        BqTemp              = Bq(InitInd:EndInd);
        BqAve               = sum(BqTemp)/numel(BqTemp);%mean(BqTemp,'all');
        QTemp               = Q(InitInd:EndInd);
        QAve                = sum(QTemp)/numel(QTemp);
        Bq(InitInd:EndInd)  = BqAve;
        Q(InitInd:EndInd)   = QAve;
        
        % Changing the layers STB based on the average of Bq and Q of layer:
