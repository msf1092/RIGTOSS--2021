function [SBTs, NormNashD] = FncSIGTOSSu2Qt(Depth, Q, Deltau2ToSigmaPrimeV0)
...
for i = 1 : n
    e = 1;
    SBT(i,e) = FncSBTpointsLocatorSchneideru2Qt(e, Deltau2ToSigmaPrimeV0(i), Q(i));
end
...
for i = 1 : n - 1
    D1(i) = (abs((Deltau2ToSigmaPrimeV0(i)) - (Deltau2ToSigmaPrimeV0(i+1)))) ^2 + (abs((Q(i)) - (Q(i+1))))^2;
end
maxD1=max(D1);
for i=1:n
...
for i=1:n-2
    D2(i) =(abs((Deltau2ToSigmaPrimeV0(i))-(Deltau2ToSigmaPrimeV0(i+2))))^2+(abs((Q(i))-(Q(i+2))))^2;
end
maxD2=max(D2);
for i=1:n
...
for i=1:n-4
    D3(i) =(abs((Deltau2ToSigmaPrimeV0(i))-(Deltau2ToSigmaPrimeV0(i+4))))^2+(abs((Q(i))-(Q(i+4))))^2;
end
maxD3=max(D3);
for i=1:n
...
for i=1:n-10
    D4(i) =(abs((Deltau2ToSigmaPrimeV0(i))-(Deltau2ToSigmaPrimeV0(i+10))))^2+(abs((Q(i))-(Q(i+10))))^2;
end
maxD4=max(D4);
for i=1:n
...
X(3,1)=0.3255;
X(4,1)=0.1691;
%to save the Deltau2ToSigmaPrimeV0 and Q values, to be exported into the final file
% Deltau2ToSigmaPrimeV01 = Deltau2ToSigmaPrimeV0;
% Q1 = Q;
%To draw SBTs:
...
for k=1:09 %13 CI levels are considered in "FncAlphaCutDist.m"
    ...
    % Finding Layers and averaging the Deltau2ToSigmaPrimeV0 and Q
    for j=1:2:m-1
        InitInd             = LayerInd(j,1);
        EndInd              = LayerInd(j+1,1);
        Deltau2ToSigmaPrimeV0Temp              = Deltau2ToSigmaPrimeV0(InitInd:EndInd);
        Deltau2ToSigmaPrimeV0Ave               = sum(Deltau2ToSigmaPrimeV0Temp)/numel(Deltau2ToSigmaPrimeV0Temp);%mean(Deltau2ToSigmaPrimeV0Temp,'all');
        QTemp               = Q(InitInd:EndInd);
        QAve                = sum(QTemp)/numel(QTemp);
        Deltau2ToSigmaPrimeV0(InitInd:EndInd)  = Deltau2ToSigmaPrimeV0Ave;
        Q(InitInd:EndInd)   = QAve;
        
        % Changing the layers STB based on the average of Deltau2ToSigmaPrimeV0 and Q of layer:
        ...
        if j==1 && LayerInd(j,e)>1
            ...
            Deltau2ToSigmaPrimeV0Temp=Deltau2ToSigmaPrimeV0(InitInd:EndInd);
            Deltau2ToSigmaPrimeV0Ave=sum(Deltau2ToSigmaPrimeV0Temp)/numel(Deltau2ToSigmaPrimeV0Temp);
            QTemp=Q(InitInd:EndInd);
            QAve=sum(QTemp)/numel(QTemp);
            Deltau2ToSigmaPrimeV0(InitInd:EndInd)=Deltau2ToSigmaPrimeV0Ave;
            Q(InitInd:EndInd)=QAve;
